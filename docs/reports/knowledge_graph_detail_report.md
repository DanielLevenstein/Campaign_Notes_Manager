...
- `language_model.lore_documents.list_characters()` reads character Markdown files from `CHARACTERS_DIR`, either as `<name>.md` files or directories containing `BACKSTORY.md`.
- `language_model.lore_documents.create_character()` writes a character sheet and regenerates that character's graph JSON.
...
- `language_model.lore_documents.list_places()` reads place Markdown files from `PLACES_DIR`.
- `language_model.lore_documents.create_place()` and `create_place_markdown()` write place Markdown files.
