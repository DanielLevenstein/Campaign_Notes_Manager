...
import src.persistence.lore_import as lore_import
...
from tests.support.test_paths import (
    LORE_IMPORT_CHARACTER_FILES,
    LORE_IMPORT_METADATA_FILES,
    LORE_IMPORT_PLACE_FILES,
    LORE_IMPORT_SESSION_NOTE_FILES,
    PROJECT_ROOT,
    TEST_FIXTURES_DIR,
)
...
def test_import_lore_directory_copies_fixture_subdirectories(tmp_path, monkeypatch):
    ...
    assert summary.characters == len(LORE_IMPORT_CHARACTER_FILES)
    assert summary.places == len(LORE_IMPORT_PLACE_FILES)
    assert summary.session_notes == len(LORE_IMPORT_SESSION_NOTE_FILES)
...
def test_clear_local_lore_cleans_configured_lore_dir(tmp_path, monkeypatch):
    ...
    assert summary.places == len(LORE_IMPORT_PLACE_FILES)
    assert summary.session_notes == len(LORE_IMPORT_SESSION_NOTE_FILES)
    assert summary.metadata == len(LORE_IMPORT_METADATA_FILES)
